...
from model.t5_try import CustomT5
...
def trainCustom(args):
    ...
    productions = grammar._productions
    prod_rules = list() # also contains Reduce, GenToken, and SelectColumnAction
    for pr in list(productions.keys()):
        rules = productions[pr]
        for rule in rules:
            rule_str = str(rule)
            if rule_str not in prod_rules:
                prod_rules.append(rule_str)
    prod_rules.append('Reduce')
    prod_rules.append('GenToken')
    prod_rules.append('SelectColumnAction')
    prod_rules = sorted(prod_rules)
    prod_rules = {v:i for i,v in enumerate(prod_rules)}

    parser_cls = Registrable.by_name(args.parser)  # TODO: add arg

    head_size = 12+5+5+6+14838 # production+primitive+field+type+src(vocab size)
    model = CustomT5(head_size=head_size)
    ...
    while True:
        ...
        for batch_examples in train_set.batch_iter(batch_size=args.batch_size, shuffle=True):
            ...
            # ret_val = model.forward(input_ids=batch_examples_modified, labels=None, prod_rules=prod_rules)
            ret_val = model.score(batch_examples, prod_rules)
            loss = -ret_val[0]
...
def testCustom(args):
    ...
    vocab = pickle.load(open("data/wikisql/vocab.bin", 'rb'))

    grammar = ASDLGrammar.from_text(open("asdl/lang/sql/sql_asdl.txt").read())
    transition_system = Registrable.by_name("sql")(grammar)

    # assert args.load_model
    head_size = 12+5+5+6+14838 # production+primitive+field+type+src(vocab size)
    model = CustomT5(head_size=head_size, args=args, vocab=vocab, transition_system=transition_system)
    
    for example in test_set.examples:
        model.parse(example=example, table=None, beam_size=5)
