...
class WikiSqlParser(Parser):
    def __init__(self, args, vocab, transition_system):
        ...
        self.t5 = T5EncoderModel.from_pretrained("t5-base").to(device)
        self.lin = nn.Linear(768, args.hidden_size*2).to(device) # for T5

    def getLast(self, question):
        # print(question)
        questions = []
        for q in question:
            questions.append(' '.join(q))
        input_ids = self.tokenizer(questions, return_tensors="pt", padding=True, truncation=True, max_length=512).to(device)
        outputs = self.t5(input_ids=input_ids['input_ids'],output_hidden_states=True)
    ...
    def score(self, examples, return_encode_state=False):
        ...
        last = self.getLast([e.src_sent for e in examples])
        last_state = last[:,0:args.hidden_size]
        last_cell = last[:,args.hidden_size:]
        # table header is passed as input and an encoding of shape table header is received
        ...
        for t in range(batch.max_action_num):
            ...
            apply_rule_prob = F.softmax(self.production_readout(att_t), dim=-1) # for each example, 0-1 prob of actions occurring (11 actions, why 12 dim then?)
            ...
            for e_id, example in enumerate(examples):
                if t < len(example.tgt_actions):
                    ...
                    if isinstance(action_t, ApplyRuleAction):
                        act_prob_t_i = apply_rule_prob[e_id, self.grammar.prod2id[action_t.production]] # ApplyRuleAction has index 10, therefore select that from apply_rule_prob
    ...
    def parse(self, question, context, beam_size=5):
        table = context
        args = self.args
        src_sent_var = nn_utils.to_input_variable([question], self.vocab.source,cuda=self.args.cuda, training=False)

        utterance_encodings, (last_state, last_cell) = self.encode(src_sent_var, [len(question)])
        last = self.getLast(' '.join(question))
        last_state = last[:,0:args.hidden_size]
        last_cell = last[:,args.hidden_size:]
